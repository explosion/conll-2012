...
"""
    ...
    sql_insert_statement = """insert into lemma_type
(
  id
)
values (%s)
...
class lemma:
    ...
    def __str__(self):
        tr = [
            "INPUT STRING:%s" % self.input_string,
    ...
    def from_db(a_leaf_id, a_cursor):
        ...
        if len(rows) != 1:
            assert all(row["lemma"] == rows[0]["lemma"] for row in rows), "\n".join(
                ", ".join(": ".join(a) for a in row.iteritems()) for row in rows
            )
    ...
    def write_to_db(self, cursor):
        data = [
            (
                self.id,
                self.input_string,
                self.b_transliteration,
                self.comment,
                self.index,
                self.offset,
                self.unvocalized_string,
                self.vocalized_string,
                self.vocalized_input,
                self.pos,
                self.gloss,
                self.lemma,
                self.coarse_sense,
...
def pretty_print_tree_string(a_tree_string, offset=""):
    ...
    def treeindent(depth):
        return (
            indent_string[depth_to_indent[depth]] + offset
        )  # Indent to appropriate point

    current_depth = 0
    for frag in close_paren.split(
        a_tree
    ):  # Split input into lines ending with a lexical item
        if frag[-1] != "\n":
            frag = frag + ")"
        else:
            frag = frag[0:-1]
        ...
        pfrag = ""
        for pfrag in (frag).split("(")[
            1:
        ]:  # Split line into segments each beginning with an '('
            ...
            depth_to_indent[current_depth] = depth_to_indent[current_depth - 1] + len(
                pfrag
            )

        current_depth = current_depth - pfrag.count(
            ")"
        )  # Correct depth given closing parens
        if current_depth <= 0:
...
def pp(sexpr, out_text=False):
    ...
    if not out_text:
        ...
        words = [
            word for tag, word in all_leaves(sexpr) if tag != "-NONE-"
        ]  # skip traces
...
def pad_items_in_list(a_list, a_character=None):
    ...
    if a_character != None:
        ...
        for i in range(0, len(a_list)):
            ...
            if (
                a_list[i].endswith("*")
                or a_list[i].endswith("-")
                or a_list[i][-1] in string.digits
            ):
                a_list[i] = "%s " % (a_list[i])
...
def start(input_fname, conll_fname, output_fname, encoding, changes):
        ...
        for a_tree in iterate_trees(inf):
            ...
            for change in changes:
                ...
                try:
                    ...
                    raise Exception(
                        "Invalid argument '%s' for change.  Allowed changes are: %s"
                        % (change, transformations.keys())
                    )
    ...
    if is_arabic is True:
        ...
        if os.path.exists(lemma_fname):
            ...
            input_string_regex = re.compile(
                r"^\s*INPUT STRING:(.*)", re.U | re.MULTILINE
            )
            ...
            unvocalized_string_regex = re.compile(
                r"^\s*UNVOCALIZED:(.*)", re.U | re.MULTILINE
            )
            vocalized_string_regex = re.compile(
                r"^\s*VOCALIZED:(.*)", re.U | re.MULTILINE
            )
            vocalized_input_string_regex = re.compile(
                r"^\s*VOC_STRING:(.*)", re.U | re.MULTILINE
            )
            ...
            while i < len(lemma_file_lines):
                input_string_regex_match = input_string_regex.findall(
                    lemma_file_lines[i]
                )

                if input_string_regex_match != []:
                    while (
                        i < len(lemma_file_lines) and lemma_file_lines[i].strip() != ""
                    ):
            ...
            for lemma_block in list_of_lemma_blocks:
                for a_list, a_regex, a_name in [
                    [list_of_input_strings, input_string_regex, "input"],
                    ...
                    [
                        list_of_unvocalized_strings,
                        unvocalized_string_regex,
                        "unvocalized_strings",
                    ],
                    [
                        list_of_vocalized_strings,
                        vocalized_string_regex,
                        "vocalized_strings",
                    ],
                    [
                        list_of_vocalized_inputs,
                        vocalized_input_string_regex,
                        "vocalized_inputs",
                    ],
            ...
            debug(
                "len(actual_word_list): %s\n" % (len(actual_word_list)),
                DEBUG,
                MAX_VERBOSITY,
            )
            debug("actual_word_list: %s\n" % (actual_word_list), DEBUG, MAX_VERBOSITY)
            debug(
                "len(buckwalter_word_list): %s\n" % (len(buckwalter_word_list)),
                DEBUG,
                MAX_VERBOSITY,
            )
            debug(
                "buckwalter_word_list: %s\n" % (buckwalter_word_list),
                DEBUG,
                MAX_VERBOSITY,
            )
            ...
            if len(actual_word_list) != len(buckwalter_word_list) or len(
                actual_word_list
            ) != len(lemma_list):
                debug(
                    "len(actual_word_list): %s\n" % (len(actual_word_list)),
                ...
                debug("len(lemma_list): %s\n" % (len(lemma_list)), DEBUG, MAX_VERBOSITY)
                raise Exception(
                    "the three lists -- actual word, buckwalter word, and lemma should be the same length, or else some information might be missing from the .lemma file"
                )

            for i in range(0, len(actual_word_list)):
                if lemma_list[i] == "DEFAULT" or buckwalter_word_list[i] == "":
                    debug(
                        "%s %s %s\n"
                        % (
                            actual_word_list[i].rjust(50),
                            buckwalter_word_list[i].rjust(50),
                            lemma_list[i].rjust(50),
                        ),
                        DEBUG,
                        MAX_VERBOSITY,
                    )
    ...
    for line in conll_file:
        if line.strip() == "":
            ...
            assert len(sentences) == len(
                w_list[i]
            ), "the example should contain the same number of words as the words in the parse"

            if a_list_of_lemmas != []:
                assert (
                    len(a_list_of_lemmas) == num_words
                ), "the list of lemmas does not match the list of words. please report this issue."
            ...
            for columns in sentences:
                ...
                if a_list_of_lemmas != []:
                    columns[WORD_COLUMN] = "%s#%s#%s#%s" % (
                        w_list[i][c],
                        a_list_of_lemmas[w].lemma.strip(),
                        a_list_of_lemmas[w].unvocalized_string.strip(),
                        a_list_of_lemmas[w].vocalized_string.strip(),
                    )
...
if __name__ == "__main__":
    ...
    if (
        ...
        and sys.argv[2] in transformations
    ):
        print("")
        ...
        print("-" * 120)
        print(
            "Usage:  python skeleton2conll.py <ontonotes-parse-file> <input-skel-file> <conll-output-file> [transformations] ..."
        )
        print("\nAllowed transforms:")
        ...
        print("Example:")
        print(
            "python skeleton2conll.py <ontonotes-release-directory>/data/.../bc/cnn/00/cnn_0000.parse conll-2011/dev/data/english/annotations/bc/cnn/00/cnn_0000.v0_gold_skel conll-2011/dev/data/english/annotations/bc/cnn/00/cnn_0000.v0_gold_conll -edited --text"
        )
        print("-" * 120)
    else:
        input_fname, conll_fname, output_fname, changes = (
            sys.argv[1],
            sys.argv[2],
            sys.argv[3],
            sys.argv[4:],
